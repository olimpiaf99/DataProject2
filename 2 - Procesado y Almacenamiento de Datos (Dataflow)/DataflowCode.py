...
class add_processing_time(beam.DoFn):
    def process(self, element):
        ...
        output_data = {'aggGlucosa': element, 'processingTime': window_start}
        output_json = json.dumps(output_data)
        yield output_json.encode('utf-8')

class agg_glucosa(beam.DoFn):
    def process(self, element):
...
def edemData(output_table):
    ...
    with beam.Pipeline(options=options) as p:
        ...
        #Part03: Count glucosa per minute and put that data into PubSub

        #Create a fixed window (1 min duration)
        (data 
            | "Get glucosa value" >> beam.ParDo(agg_glucosa())
